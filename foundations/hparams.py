        ...
        for field in fields(cls):
            ...
            if field.type == bool:
                if (
                    ...
                    raise ValueError(
                        f"Boolean hyperparameters must default to False: {field.name}."
                    )
                parser.add_argument(
...
class DatasetHparams(Hparams):
    ...
    _description: str = (
        "Hyperparameters that select the dataset, data augmentation, and other data transformations."
    )
    ...
    _unsupervised_labels: str = (
        "Replace the standard labels with alternative, unsupervised labels. Example: rotation"
    )
    _blur_factor: str = (
...
class ModelHparams(Hparams):
    ...
    _model_init: str = (
        "The model initializer. Examples: kaiming_normal, kaiming_uniform, binary, orthogonal"
    )
    _batchnorm_init: str = "The batchnorm initializer. Examples: uniform, fixed"
    _batchnorm_frozen: str = (
        "If True, all batch normalization parameters are frozen at initialization."
    )
    _output_frozen: str = (
        "If True, all outputt layer parameters are frozen at initialization."
    )
    _others_frozen: str = (
        "If true, all other (non-output, non-batchnorm) parameters are frozen at initialization."
    )
    _others_frozen_exceptions: str = (
        "A comma-separated list of any tensors that should not be frozen."
    )
...
class TrainingHparams(Hparams):
    ...
    _nesterov: bool = (
        "The nesterov momentum to use with the SGD optimizer. Cannot set both momentum and nesterov."
    )
    ...
    _data_order_seed: str = (
        "The random seed for the data order. If not set, the data order is random and unrepeatable."
    )
    _warmup_steps: str = (
        "Steps of linear lr warmup at the start of training as epochs ('20ep') or iterations ('800it')"
    )
    _weight_decay: str = "The L2 penalty to apply to the weights."
    _apex_fp16: bool = (
        "Whether to train the model in float16 using the NVIDIA Apex library."
    )
...
class PruningHparams(Hparams):
    ...
    _description: str = (
        "Hyperparameters that determine how the model is pruned. "
