...
class TestCifarVGG(test_case.TestCase):
    ...
    def test_vgg11(self):
        model = cifar_vgg.Model.get_model_from_name(
            "cifar_vgg_11", initializers.kaiming_normal
        )
        self.assertEqual(self.count_parameters(model), 9231114)

    def test_vgg13(self):
        model = cifar_vgg.Model.get_model_from_name(
            "cifar_vgg_13", initializers.kaiming_normal
        )
        self.assertEqual(self.count_parameters(model), 9416010)

    def test_vgg16(self):
        model = cifar_vgg.Model.get_model_from_name(
            "cifar_vgg_16", initializers.kaiming_normal
        )
        self.assertEqual(self.count_parameters(model), 14728266)

    def test_vgg19(self):
        model = cifar_vgg.Model.get_model_from_name(
            "cifar_vgg_19", initializers.kaiming_normal
        )
