...
class Mask(dict):
    ...
    def __setitem__(self, key, value):
        ...
        if not isinstance(value, torch.Tensor):
            raise ValueError(
                "value for key {} must be torch Tensor or numpy ndarray.".format(key)
            )
    ...
    def save(self, output_location):
        ...
        get_platform().save_model(
            {k: v.cpu().int() for k, v in self.items()}, paths.mask(output_location)
        )
        ...
        with get_platform().open(paths.sparsity_report(output_location), "w") as fp:
            fp.write(
                json.dumps(
                    {"total": float(total_weights), "unpruned": float(total_unpruned)},
                    indent=4,
                )
            )
    ...
    def sparsity(self):
        ...
        total = torch.sum(
            torch.tensor([torch.sum(torch.ones_like(v)) for v in self.values()])
        )
