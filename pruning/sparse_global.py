...
class PruningHparams(hparams.PruningHparams):
    ...
    _layers_to_ignore = (
        "A comma-separated list of addititonal tensors that should not be pruned."
    )
...
class Strategy(base.Strategy):
    ...
    def prune(
        ...
        current_mask = (
            Mask.ones_like(trained_model).numpy()
            if current_mask is None
            else current_mask.numpy()
        )

        # Determine the number of weights that need to be pruned.
        number_of_remaining_weights = np.sum([np.sum(v) for v in current_mask.values()])
        number_of_weights_to_prune = np.ceil(
            pruning_hparams.pruning_fraction * number_of_remaining_weights
        ).astype(int)
