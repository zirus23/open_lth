...
class TestRunner(test_case.TestCase):
    ...
    def to_step(self, s):
        return Step.from_str(
            s, datasets.registry.iterations_per_epoch(self.desc.dataset_hparams)
        )
    ...
    def test_level0_2it(self):
        ...
        self.assertLevelFilesPresent(
            level_root, self.to_step("0it"), self.to_step("2it")
        )
    ...
    def test_level3_2it(self):
        ...
        for level in range(0, 4):
            ...
            self.assertTrue(
                np.allclose(
                    pct,
                    sparsity_report["unpruned"] / sparsity_report["total"],
                    atol=0.01,
                )
            )
            ...
            self.assertStateEqual(
                level_weights,
                {k: v * mask.get(k, 1) for k, v in level0_weights.items()},
            )
    ...
    def test_level3_4it_pretrain2it(self):
        ...
        self.assertLevelFilesPresent(
            pretrain_root, self.to_step("0it"), self.to_step("2it"), masks=False
        )

        # Load the pretrain and level0 start weights to ensure they're the same.
        pretrain_end_weights = paths.model(
            self.desc.run_path(2, "pretrain"), self.desc.pretrain_end_step
        )
        pretrain_end_weights = {
            k: v.numpy() for k, v in torch.load(pretrain_end_weights).items()
        }

        level0_weights = paths.model(
            self.desc.run_path(2, 0), self.desc.train_start_step
        )
        level0_weights = {k: v.numpy() for k, v in torch.load(level0_weights).items()}
        ...
        for level in range(0, 2):
            level_root = self.desc.run_path(2, level)
            self.assertLevelFilesPresent(
                level_root, self.to_step("2it"), self.to_step("4it")
            )
    ...
    def test_level3_4it_pretrain2it_different_output_size(self):
        ...
        output_layer_names = models.registry.get(
            self.desc.model_hparams
        ).output_layer_names
        self.assertStateEqual(
            {
                k: v
                for k, v in pretrain_end_weights.items()
                if k not in output_layer_names
            },
