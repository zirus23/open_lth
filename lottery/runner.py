...
class LotteryRunner(Runner):
    ...
    def run(self) -> None:
        if self.verbose and get_platform().is_primary_process:
            ...
            print(
                f"Output Location: {self.desc.run_path(self.replicate, 0)}"
                + "\n"
                + "=" * 82
                + "\n"
            )
    ...
    def _pretrain(self):
        ...
        model = models.registry.get(
            self.desc.model_hparams, outputs=self.desc.pretrain_outputs
        )
    ...
    def _establish_initial_weights(self):
        ...
        new_model = models.registry.get(
            self.desc.model_hparams, outputs=self.desc.train_outputs
        )
        ...
        if self.desc.pretrain_training_hparams is not None:
            ...
            if self.desc.train_outputs != self.desc.pretrain_outputs:
                state_dict.update(
                    {k: new_model.state_dict()[k] for k in new_model.output_layer_names}
                )
    ...
    def _train_level(self, level: int):
        ...
        model = models.registry.load(
            self.desc.run_path(self.replicate, 0),
