...
class LotteryDesc(Desc):
    ...
    def add_args(parser: argparse.ArgumentParser, defaults: "LotteryDesc" = None):
        ...
        if pruning_strategy:
            ...
            if (
                defaults
                and defaults.pruning_hparams.pruning_strategy == pruning_strategy
            ):
        ...
        if pretrain:
            if defaults:
                def_th = replace(defaults.training_hparams, training_steps="0ep")
            hparams.TrainingHparams.add_args(
                parser,
    ...
    def create_from_args(cls, args: argparse.Namespace) -> "LotteryDesc":
        ...
        if args.pretrain and not Step.str_is_zero(args.pretrain_training_steps):
            ...
            desc.pretrain_dataset_hparams._name = (
                "Pretraining " + desc.pretrain_dataset_hparams._name
            )
    ...
    def str_to_step(self, s: str, pretrain: bool = False) -> Step:
        dataset_hparams = (
            self.pretrain_dataset_hparams if pretrain else self.dataset_hparams
        )
    ...
    def run_path(
        self, replicate: int, pruning_level: Union[str, int], experiment: str = "main"
    ):
