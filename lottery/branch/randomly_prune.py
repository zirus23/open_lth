    ...
    def branch_function(
        ...
        # Randomize evenly across all layers.
            ...
            for i, k in sorted(mask.keys()):
                layer_mask = torch.where(
                    torch.arange(mask[k].size) < torch.ceil(sparsity * mask[k].size),
                    torch.ones_like(mask[k].size),
                    torch.zeros_like(mask[k].size),
                )
                mask[k] = shuffle_tensor(layer_mask, seed=seed + i).reshape(
                    mask[k].size
                )
        ...
        model = PrunedModel(
            models.registry.load(
                self.level_root, state_step, self.lottery_desc.model_hparams
            ),
            mask,
        )
